...
fn resolve_clojure_require(
    ...
    let target = target_ids.iter().find(|id| {
        entity_map.get(*id).map_or(false, |e| {
            let fp = &e.file_path;
            fp.ends_with(suffix_clj.as_str())
                || fp.ends_with(suffix_cljs.as_str())
                || fp.ends_with(suffix_cljc.as_str())
        })
    });
    if let Some(target_id) = target {
        local_imports.push((
...
// `[^[]*` ensures we don't cross into the :refer [...] bracket.
static CLOJURE_REFER_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\[([a-z][a-z0-9._-]*)\b[^\[]*:refer\s+\[([^\]]+)\]").unwrap());
...
static CLOJURE_AS_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\[([a-z][a-z0-9._-]*)\b[^\]]*:as\s+([a-zA-Z][a-zA-Z0-9_-]*)").unwrap()
});

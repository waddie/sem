...
fn visit_node(
    ...
    while let Some((node, pid_owned, sup_owned)) = worklist.pop() {
        ...
        // is a defining keyword (defn, ns, defrecord, etc.). We never recurse into
        // list_lit bodies — all Clojure definitions live at the top level of the file.
        ...
        if node_type == "list_lit" && !config.call_entity_identifiers.is_empty() {
            ...
            // Always skip children: Clojure definitions are always top-level list
            // forms and never nest meaningfully inside other list forms. Recursing
            // would only find inner let/when/do bodies, which are not entities.
            continue;
        }
